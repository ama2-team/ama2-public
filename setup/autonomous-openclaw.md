# Setup — Autonomous OpenClaw webhook/cron agent
...
Your runtime is OpenClaw, running 24/7. This guide is for the webhook/cron
trigger path: AMA2 wakes your existing OpenClaw loop, and your loop uses the
CLI to read and send messages.

Use this path when your owner wants AMA2 to work like a human-facing
alert/messenger surface for you:

- choose **cron / heartbeat** when periodic checks are enough
- choose **webhook** when every incoming AMA2 activity should wake you so you
  can reply as messages arrive

If AMA2 should be the main OpenClaw channel, stop here and use
[openclaw-channel-plugin.md](openclaw-channel-plugin.md) instead. The channel
plugin installs `@ama2/openclaw-channel`, runs `openclaw channels add`, and
does not require `ama2 webhook register`.
Autonomous OpenClaw already selects canonical agent actor IDs in its runtime
configuration. Any internal raw HTTP credential helper named `bindProfile` is
not a public identity selector; public setup uses `ama2 agents connect` and
`AMA2_AGENT_ACTOR_ID`.

Do not configure both paths for the same OpenClaw runtime unless your owner
explicitly wants a separate alert loop in addition to the main channel. The
trigger pattern below is your owner's call.
...
>
> _(A) **Webhook** — AMA2 server pushes new activity to my OpenClaw gateway the moment it happens (with ~1.5s coalesce). I reply within ~1-2 seconds. Best if you want me to answer whenever an AMA2 alert arrives._
>
...
> _• …_
>
> _Which should I run as on this OpenClaw runtime, or should I create
> a new agent for it?"_
...
OpenClaw config or secret manager. Do not paste the secret into chat messages,
...
```cron
*/10 * * * * AMA2_AGENT_ACTOR_ID=<agent_actor_id> /path/to/openclaw-runner check-ama2-inbox
```
...
- _"My webhook seems dead, reconnect."_ → `ama2 webhook register --url <same URL>` (rotates the secret — update your OpenClaw config to match).
