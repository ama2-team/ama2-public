...
# roster: "<profile>\t<identity_dir>\t<workspace_dir>\t<engine>" per member.
roster() {
...
for m in data.get("members", []):
    ...
    profile, d = m.get("profile"), m.get("dir")
    if profile and d:
        # \x1f (unit separator, non-whitespace) so EMPTY fields aren't collapsed
        print(f"{profile}\x1f{d}\x1f{m.get('workspace_dir','')}\x1f{m.get('engine','')}")
PY
}
...
import json, sys
f, prof, eng = sys.argv[1], sys.argv[2], sys.argv[3]
d = json.load(open(f))
for m in d.get("members", []):
    if m.get("profile") == prof: m["engine"] = eng
with open(f, "w") as fh:
    json.dump(d, fh, indent=2, ensure_ascii=False); fh.write("\n")
PY
}

resolve_engine() {  # echo engine for $1=profile given $2=current(maybe empty)
  ...
  save_engine "$profile" "$engine"
  echo "$engine"
}

start_one() {
  local name="$1" idir="$2" profile="$3" workspace="$4" engine="$5"
  ...
  nohup "$ROOT/scripts/poll-loop.sh" "$idir" "$profile" "$INTERVAL" "$workspace" "$engine" \
...
started=0
while IFS=$'\x1f' read -r profile dir workspace engine; do
  [ -n "$profile" ] || continue
  engine="$(resolve_engine "$profile" "$engine")"
  start_one "$profile" "$ROOT/$dir" "$profile" "$workspace" "$engine"
  started=$((started + 1))
done < <(roster)
