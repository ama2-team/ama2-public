...
This command is designed to run weekly. The owner may schedule it via Claude Code's task scheduler — when run on Monday at 7am ET, the output goes straight to their drive and (if configured) Slack/Teams DM channel.
